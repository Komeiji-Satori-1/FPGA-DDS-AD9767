...
第一版建议采用简单实现：读数据在同一帧的 DATA 阶段移出。如果实现上存在时序压力，也可以改为下一帧返回，但必须在 MCU 协议文档中明确。
...
1. SPI 读数据是否必须同帧返回，还是可以下一帧返回。
2. `phase_reset` 是保持型位还是写 1 触发后自清零。
3. 按键控制和 SPI 控制同时使能时，谁优先。
4. 第一版是否需要实现 `AMP_SCALE` 和 `DC_OFFSET`，还是只预留寄存器。
